...
void HHeightMap::Init(int& _X, int& _Z, const HGAMESTRING _MapHeightTex, int _Order)
{
	// HVECTOR Test = HVECTOR::Corss3D(HVECTOR::RIGHT, HVECTOR::FORWARD);
	...
	m_Render = ACTOR()->CreateCom<HRENDERER>(_Order);
	...
	MapOption.PixelUv = HVECTOR(1.0f / m_HTex->Size().x, 1.0f / m_HTex->Size().y);
	MapOption.PixelScale = HVECTOR(TRANS()->WSCALE().x / m_HTex->Size().x, TRANS()->WSCALE().z / m_HTex->Size().y);
	...
	for (int z = 0; z < Z + 1; z++)
	{
		for (int x = 0; x < X + 1; x++)
		{
			...
			m_VecVtx[Index].Normal = HVECTOR::UP;
		}
	}
	...
	//		HVECTOR Normal = HVECTOR::Corss3D((UpPos0 - UpPos1) , (UpPos0 - UpPos2));
	//		Normal.NORMAL3D();
	//		Normal.w = 0.0f;
	...
	//		Normal = HVECTOR::Corss3D((DownPos0 - DownPos1), (DownPos0 - DownPos2));
	//		Normal.NORMAL3D();
	//		Normal.w = 0.0f;
	//		m_VecVtx[Index].Normal += Normal;
	//		m_VecVtx[Index].Normal = m_VecVtx[Index].Normal / 2.0f;
...
void HHeightMap::BaseTexture(const HGAMESTRING _TextureName) 
{
	m_Render->TEXTURE(L"BaseTex", _TextureName);
}

HVECTOR HHeightMap::CalTexelPos(const HVECTOR& _Pos)
{
	HVECTOR Size = m_HTex->Size();
...
void HHeightMap::HTex(const HGAMESTRING _TextureName) 
{
	...
	for (int z = -HZ; z < (HZ + 1); z++)
	{
		for (int x = -HX; x < (HX + 1); x++)
		{
			...
			// 이 둘을 외적하면 하면
			//HVECTOR Dir1 = m_YTileMap[Index.Key].Up.Arr[0] - m_YTileMap[Index.Key].Up.Arr[1];
			//HVECTOR Dir2 = m_YTileMap[Index.Key].Up.Arr[0] - m_YTileMap[Index.Key].Up.Arr[2];
...
HVECTOR HHeightMap::CalPosToWorld(const HVECTOR& _Pos) 
{
	HVECTOR ReturnX;
	HVECTOR Scale;
	Scale = TRANS()->WSCALE();
...
float HHeightMap::YHeight(float4 _Pos) 
{
	...
	HVECTOR Ori = _Pos;
	Ori.y = 0.0f;
	HVECTOR Dir = HVECTOR::UP;


	int2 Key = Index(_Pos);
	float YDis = Min;

	HVECTOR UpTri0 = CalPosToWorld(m_YTileMap[Key.Key].Up.Arr[0]);
	HVECTOR UpTri1 = CalPosToWorld(m_YTileMap[Key.Key].Up.Arr[1]);
	HVECTOR UpTri2 = CalPosToWorld(m_YTileMap[Key.Key].Up.Arr[2]);
	...
	HVECTOR DownTri0 = CalPosToWorld(m_YTileMap[Key.Key].Down.Arr[0]);
	HVECTOR DownTri1 = CalPosToWorld(m_YTileMap[Key.Key].Down.Arr[1]);
	HVECTOR DownTri2 = CalPosToWorld(m_YTileMap[Key.Key].Down.Arr[2]);
