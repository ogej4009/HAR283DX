...
public:
	...
	std::vector<std::vector<HPTR<HMESH>>> m_Mesh;
...
public:
	...
	HGAMESTRING DifTexture;	// 텍스처경로 
	HGAMESTRING BmpTexture; // 텍스처경로
	HGAMESTRING SpcTexture; // 텍스처경로
...
struct HFbxExMeshInfo
{
	...
	int MaterialNum;
	bool bIsSkelMesh;
	HGAMESTRING SkeletonRoot;
	int SkeletonElemNum;
	bool bIsLodGroup;
	HGAMESTRING LODGroup;
	int LODLevel;
	int MorphNum;
...
struct JointPos
{
	...
	void SetFromMatrix(const HMATRIX& InMatrix)
	{
		...
		Rotation = HVECTOR::MatrixToQuaternion(M);
...
class HIBBUFFER;
class HRenderTarget;
class HFbxEx : public HFILEMGR<HFbxEx>
{
public:
	static HPTR<HFbxEx> Load(const HGAMEFILE& _File, float4 _Angle = float4::ZERO)
	{
		HPTR<HFbxEx> Res = CreateToInsertToFile(_File.FullPath());
		Res->Load(_Angle);
		return Res;
	}
...
private:
	...
	HGAMESTRING MaterialTex(fbxsdk::FbxSurfaceMaterial* pMtrl, const char* _FactorName);
	...
	Bone* FindBone(const HGAMESTRING& _boneName);
...
public:
	void CalHFbxExBoneFrameTransMatrix(HPTR<HFbxEx> _Fbx);

	int GetDrawSetCount();
	DRAWSET* GetDrawSet(int LodLevel);
	size_t GetBoneCount();
	Bone* FindBone(const int Index);
	HGAMESTRING GetMeshNameByIndex(const int Index);
...
private:
	void ProcessAnimationLoad(HPTR<HFbxEx> _Fbx, fbxsdk::FbxNode* pNode);
	void ProcessAnimationCheckState(HPTR<HFbxEx> _Fbx);
	void MeshAnimationLoad(HPTR<HFbxEx> _Fbx, fbxsdk::FbxNode* pNode);
