...
void HBoneAnimationCom::MESHANI::UpdateNext()
{
	...
	for (int i = 0; i < (int)m_Fbx->m_UserBoneData.size(); ++i)
	{
		...
		HVECTOR S = HVECTOR::Lerp(CurFrameData.S, NextFrameData.S, Percent);
		// Q는 쿼터니온
		...
		// 회전도 중간값으로 보간되고
		// 구면선형보간을 사용한다.
		HVECTOR Q = HVECTOR::QSLerp(CurFrameData.Q, NextFrameData.Q, Percent);
		HVECTOR T = HVECTOR::Lerp(CurFrameData.T, NextFrameData.T, Percent);
...
HPTR<HBoneAnimationCom::MESHANI> HBoneAnimationCom::FindAni(const HGAMESTRING& _AniName)
{
...
void HBoneAnimationCom::ChangeAni(const HGAMESTRING& _AniName)
{
...
std::list<HPTR<HRENDERPLAYER>> HBoneAnimationCom::CreateAni(
	const HGAMESTRING& _Fbx, 		 // const HGAMESTRING& _Fbx				Monster03.fbx 에서
	const HGAMESTRING& _TakeName,	 // const HGAMESTRING& _TakeName 		Take 001로 1434
	const HGAMESTRING& _AniName,	 // const HGAMESTRING& _AniName 		Attack을 만들어라.
	int _Start,						 // 500
	int _End,						 // 580
	const HGAMESTRING& _MatName ,	 // 를 어떤 메테리얼로 랜더링 해라.
	float _Speed ,					// 속력으로
	bool _Loop 				// 속력으로
)
{
	HPTR<HFBX> FbxPtr = HFBX::Find(_Fbx);
	...
	std::map<HGAMESTRING, std::vector<HPTR<HRENDERPLAYER>>>::iterator FindRenderPlayer = m_AllRenderPlayer.find(_Fbx);
	if (m_AllRenderPlayer.end() == m_AllRenderPlayer.find(_Fbx))
	{
		...
		m_AniBoneTex[_Fbx]->Create(
			HVECTOR((float)(FbxPtr->m_UserBoneData.size() * 4.0F), 1.0F),  // 크기
	...
	return std::list<HPTR<HRENDERPLAYER>>();
}
