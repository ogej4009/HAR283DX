...
void HFbxEx::Load(float4 modifyRotationAxis)
{
...
void HFbxEx::ImportControlPoints()
{
...
void HFbxEx::ImportBone()
{
	...
	for (LinkIndex = 0; LinkIndex < SortedLinks.size(); ++LinkIndex)
	{
		...
		for (size_t AltLinkIndex = LinkIndex + 1; AltLinkIndex < SortedLinks.size(); ++AltLinkIndex)
		{
			...
			HGAMESTRING tempLinkName = Link->GetName();
			HGAMESTRING tempAltLinkName = AltLink->GetName();
	...
	HGAMESTRING LinksWithoutBindPoses;
	int NumberOfRoot = 0;
...
void HFbxEx::LoadSkinAndCluster()
{
	std::map<int, DRAWSET*>::iterator iter = m_mapDrawsets.begin();
	std::map<int, DRAWSET*>::iterator iterEnd = m_mapDrawsets.end();
	for (; iter != iterEnd; ++iter)
...
void HFbxEx::DrawSetMeshesCreate()
{
	std::map<int, DRAWSET*>::iterator iter = m_mapDrawsets.begin();
	std::map<int, DRAWSET*>::iterator iterEnd = m_mapDrawsets.end();
	for (; iter != iterEnd; ++iter)
...
fbxsdk::FbxNode* HFbxEx::RecursiveFindParentLodGroup(fbxsdk::FbxNode* parentNode)
{
	if (nullptr == parentNode)
	{
		return nullptr;
...
fbxsdk::FbxNode* HFbxEx::FindLODGroupNode(fbxsdk::FbxNode* NodeLodGroup, int LodIndex, fbxsdk::FbxNode* NodeToFind)
{
...
fbxsdk::FbxNode* HFbxEx::RecursiveGetFirstMeshNode(fbxsdk::FbxNode* Node, fbxsdk::FbxNode* NodeToFind)
{
	if (Node == nullptr)
	{
		return nullptr;
...
void HFbxEx::TraverseHierarchyNodeRecursively(HFbxExSceneInfo& SceneInfo, fbxsdk::FbxNode* ParentNode, HFbxExNodeInfo& ParentInfo)
{
...
fbxsdk::FbxAMatrix HFbxEx::ComputeTotalMatrix(fbxsdk::FbxNode* Node)
{
...
bool HFbxEx::IsOddNegativeScale(const fbxsdk::FbxAMatrix& TotalMatrix)
{
...
void HFbxEx::DrawSetMaterialSetting(fbxsdk::FbxNode* _Node, DRAWSET* _DrawData)
{
...
float4 HFbxEx::MaterialColor(fbxsdk::FbxSurfaceMaterial* pMtrl, const char* _ColorName, const char* _FactorName)
{
...
float HFbxEx::MaterialFactor(fbxsdk::FbxSurfaceMaterial* pMtrl, const char* _FactorName) {
	double dFactor = 0;
	FbxProperty FactorPro = pMtrl->FindProperty(_FactorName);
...
HGAMESTRING HFbxEx::MaterialTex(fbxsdk::FbxSurfaceMaterial* pMtrl, const char* _FactorName)
{
...
void HFbxEx::LoadUv(fbxsdk::FbxMesh* _Mesh, std::vector<VTX3DMESHIN>& _ArrVtx, int VtxId, int _Index)
{
	std::map<HGAMESTRING, int> UVSets;
	...
	for (int uvLayerIndex = 0; uvLayerIndex < layerCount; ++uvLayerIndex)
	{
		...
		if (0 != uvSetCount)
		{
			...
			for (int UVIndex = 0; UVIndex < uvSetCount; ++UVIndex)
			{
				...
				{
					HGAMESTRING uvName = elementUv->GetName();
					if (true == uvName.IsEmpty())
					{
						uvName.Appendf("UVmap_%d", uvLayerIndex);
					}
					std::map<HGAMESTRING, int>::iterator findIter = UVSets.find(uvName);
...
void HFbxEx::LoadBinormal(fbxsdk::FbxMesh* _Mesh, fbxsdk::FbxAMatrix _MeshMatrix, std::vector<VTX3DMESHIN>& _ArrVtx, int VtxId, int _Index)
{
...
void HFbxEx::LoadTangent(fbxsdk::FbxMesh* _Mesh, fbxsdk::FbxAMatrix _MeshMatrix, std::vector<VTX3DMESHIN>& _ArrVtx, int VtxId, int _Index)
{
...
void HFbxEx::LoadNormal(fbxsdk::FbxMesh* _Mesh, fbxsdk::FbxAMatrix _MeshMatrix, std::vector<VTX3DMESHIN>& _ArrVtx, int VtxId, int _Index)
{
...
void HFbxEx::DrawSetMeshCreate(DRAWSET* _DrawData)
{
...
void HFbxEx::LoadAnimationVertexData(DRAWSET* _DrawData, const std::vector<ClusterData>& vecClusterData)
{
	for (auto& clusterData : vecClusterData)
	{
		...
		HGAMESTRING StrBoneName = clusterData.LinkName;
...
void HFbxEx::CalAnimationVertexData(DRAWSET* _DrawData)
{
...
void HFbxEx::DrawSetWeightAndIndexSetting(DRAWSET* _DrawSet, fbxsdk::FbxMesh* _Mesh, fbxsdk::FbxCluster* _Cluster, int _BoneIndex)
{
...
Bone* HFbxEx::FindBone(const HGAMESTRING& _boneName)
{
...
Bone* HFbxEx::FindBone(const int Index)
{
...
int HFbxEx::GetFbxMeshCount()
{
	return (int)m_SceneInfo.MeshInfo.size();
}

HGAMESTRING HFbxEx::GetMeshNameByIndex(const int Index)
{
...
bool HFbxEx::RetrievePoseFromBindPose(fbxsdk::FbxScene* pScene, const std::vector<fbxsdk::FbxNode*>& NodeArray, fbxsdk::FbxArray<fbxsdk::FbxPose*>& PoseArray)
{
	...
	for (int PoseIndex = 0; PoseIndex < PoseCount; PoseIndex++)
	{
		...
		if (CurrentPose && CurrentPose->IsBindPose())
		{
			HGAMESTRING PoseName = CurrentPose->GetName();
			fbxsdk::FbxStatus Status;

			for (auto Current : NodeArray)
			{
				HGAMESTRING CurrentName = Current->GetName();
				...
				else
				{
					...
					else
					{
						...
						{
							HGAMESTRING ErrorString = Status.GetErrorString();
							HGAMESTRING CurrentName = Current->GetName();
...
void HFbxEx::BuildSkeletonSystem(fbxsdk::FbxScene* pScene, std::vector<fbxsdk::FbxCluster*>& ClusterArray, std::vector<fbxsdk::FbxNode*>& OutSortedLinks)
{
	fbxsdk::FbxNode* Link;
...
fbxsdk::FbxNode* HFbxEx::GetRootSkeleton(fbxsdk::FbxScene* pScene, fbxsdk::FbxNode* Link)
{
...
void HFbxEx::RecursiveBuildSkeleton(fbxsdk::FbxNode* Link, std::vector<fbxsdk::FbxNode*>& OutSortedLinks)
{
	if (IsBone(Link))
	{
...
bool HFbxEx::IsBone(fbxsdk::FbxNode* Link)
{
...
bool HFbxEx::IsNull(fbxsdk::FbxNode* Link)
{
	fbxsdk::FbxNodeAttribute* Attr = Link->GetNodeAttribute();
	if (Attr)
	{
...
int HFbxEx::GetDrawSetCount() 
{
	return (int)m_mapDrawsets.size();
}

DRAWSET* HFbxEx::GetDrawSet(int LodLevel)
{
...
HMATRIX HFbxEx::FbxMatToHMATRIX(const fbxsdk::FbxAMatrix& _BaseTrans)
{
	HMATRIX Mat;
...
float4 HFbxEx::FbxVecTofloat4(const fbxsdk::FbxVector4& _BaseVector)
{
	float4 Vec;
...
float4 HFbxEx::FbxVecToTransform(const fbxsdk::FbxVector4& _BaseVector)
{
	float4 Vec;
	Vec.Arr[0] = (float)_BaseVector.mData[0];
	Vec.Arr[1] = (float)_BaseVector.mData[1];
...
float4 HFbxEx::FbxQuaternionTofloat4(const fbxsdk::FbxQuaternion& _BaseQ)
{
	float4 Vec;
...
bool HFbxEx::IsSkeleton()
{
	return true;
}
bool HFbxEx::IsAnimation()
{
	return 0 != m_UserAniData.size();
}

void HFbxEx::CalHFbxExBoneFrameTransMatrix(HPTR<HFbxEx> _Fbx)
{
...
void HFbxEx::ProcessAnimationLoad(HPTR<HFbxEx> _Fbx, fbxsdk::FbxNode* pNode)
{
...
void HFbxEx::ProcessAnimationCheckState(HPTR<HFbxEx> _Fbx)
{
...
void HFbxEx::MeshAnimationLoad(HPTR<HFbxEx> _Fbx, fbxsdk::FbxNode* pNode)
{
	...
	for (int deformerIndex = 0; deformerIndex < deformerCount; ++deformerIndex)
	{
		...
		for (size_t userAniDataIndex = 0; userAniDataIndex < userAniDataSize; ++userAniDataIndex)
		{
			...
			for (unsigned int clusterIndex = 0; clusterIndex < numOfClusters; ++clusterIndex)
			{
				...
				HGAMESTRING currJointName = pCurrCluster->GetLink()->GetName();
				...
				for (fbxsdk::FbxLongLong i = startTime; i <= endTime; ++i)
				{
					...
					HGAMESTRING linkName = pCurrCluster->GetLink()->GetName();
...
fbxsdk::FbxAMatrix HFbxEx::GetGeometryTransformation(fbxsdk::FbxNode* pNode)
{
