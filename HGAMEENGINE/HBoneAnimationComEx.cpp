...
bool HBoneAnimationComEx::MESHANI::IsEnd()
{
	return m_CurFrame == m_End || m_CurFrame == m_End + 1;
}

void HBoneAnimationComEx::MESHANI::Reset()
{
...
void HBoneAnimationComEx::MESHANI::SetStartFrame(int _Frame)
{
	if (m_End < _Frame)
	{
		return;
...
void HBoneAnimationComEx::MESHANI::SetEndFrame(int _Frame)
{
	if (m_Start > _Frame)
	{
		return;
...
void HBoneAnimationComEx::MESHANI::Update()
{
	// 시간 재서
	// 프레임을 전진시킨다.
	// I + 1
...
void HBoneAnimationComEx::MESHANI::UpdateNext()
{
	// Update 다음에서.
	// 100 101
	// 텍스처에 담아줘야할 본이 몇개죠?
	...
	for (int i = 0; i < (int)FrameMatData.size(); ++i)
	{
		...
		HVECTOR S = HVECTOR::Lerp(CurFrameData.S, NextFrameData.S, 0.0F);
		HVECTOR Q = HVECTOR::QSLerp(CurFrameData.Q, NextFrameData.Q, 0.0F);
		HVECTOR T = HVECTOR::Lerp(CurFrameData.T, NextFrameData.T, 0.0F);
...
HPTR<HRENDERPLAYER> HBoneAnimationComEx::FbxToPartRenderPlayer(const HGAMESTRING& _Fbx, const HGAMESTRING& _Mat, int _DrawSetIndex, int _PushListIndex, int _MainIndex, int _SubIndex)
{
	// 정말 한부만 사용하려는 경우가 있다면

	if (nullptr != m_MainFbx)
	{
		HPTR<HRENDERPLAYER> Rp = m_Render->CreateRenderPlayerToFbxPartEx(_Fbx, _Mat, _DrawSetIndex, _MainIndex, _SubIndex);
...
std::vector<HPTR<HRENDERPLAYER>> HBoneAnimationComEx::MainFbx(const HGAMESTRING& _Fbx, const HGAMESTRING& _Mat, int _Index)
{
	if (nullptr != m_MainFbx)
	{
	...
	std::vector<HPTR<HRENDERPLAYER>> Players = m_Render->CreateRenderPlayerToFbxEx(_Fbx, _Mat, _Index);
	...
	m_BoneTex->Create(
			HVECTOR((float)(m_MainFbx->GetBoneCount() * 4.0F), 1.0F),  // 크기
...
HPTR<HBoneAnimationComEx::MESHANI> HBoneAnimationComEx::FindAni(const HGAMESTRING& _AniName)
{
...
void HBoneAnimationComEx::SetCurFrame(int _Frame)
{
	if (nullptr != m_CurAni)
	{
...
void HBoneAnimationComEx::ChangeAni(const HGAMESTRING& _AniName)
{
	// 이제 해야할 애니메이션
...
void HBoneAnimationComEx::CreateAni(
	const HGAMESTRING& _AniFbx, 		 // const HGAMESTRING& _Fbx				Monster03.fbx 에서
	const HGAMESTRING& _AniName,	 // const HGAMESTRING& _AniName 		Attack을 만들어라.
	int _Start,						 // 500
	int _End,						 // 580
...
{
	HPTR<HFbxEx> FbxPtr = HFbxEx::Find(_AniFbx);
	...
	// 애니메이션이 있는쪽이 호출해야 한다.
	FbxPtr->CalHFbxExBoneFrameTransMatrix(m_MainFbx);

	HPTR<MESHANI> NewAni = new MESHANI();
...
void HBoneAnimationComEx::Init()
{
	m_Render = ACTOR()->FindComType<HRENDERER>();
	m_IsPlay = true;
	if (nullptr == m_Render)
	{
...
void HBoneAnimationComEx::Update()
{
	if (nullptr == m_CurAni)
	{
		return;
...
void HBoneAnimationComEx::UpdateNext()
{
	if (nullptr == m_CurAni)
	{
		return;
