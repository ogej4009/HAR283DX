...
void HTEXTURE::Load() 
{
	...
	{
		// m_SRV
		if (S_OK != DirectX::CreateShaderResourceView(HGAMEDEVICE::MAINOBJ()->Device(), m_Img.GetImages(), m_Img.GetImageCount(), m_Img.GetMetadata(), &m_SRV))
		{
...
HVECTOR HTEXTURE::Size() 
{
	return HVECTOR((float)Desc.Width, (float)Desc.Height, 1.0F );
}
...
void HTEXTURE::Reset(SHADERTYPE _Type, unsigned int _Reg) 
{
	...
	{
	case SHADERTYPE::SHADER_VS:
		HGAMEDEVICE::MAINOBJ()->Context()->VSSetShaderResources(_Reg, 1, &NULLPTR);
		break;
	case SHADERTYPE::SHADER_HS:
		HGAMEDEVICE::MAINOBJ()->Context()->HSSetShaderResources(_Reg, 1, &NULLPTR);
		break;
	case SHADERTYPE::SHADER_DS:
		HGAMEDEVICE::MAINOBJ()->Context()->DSSetShaderResources(_Reg, 1, &NULLPTR);
		break;
	case SHADERTYPE::SHADER_GS:
		HGAMEDEVICE::MAINOBJ()->Context()->GSSetShaderResources(_Reg, 1, &NULLPTR);
		break;
	case SHADERTYPE::SHADER_PS:
		HGAMEDEVICE::MAINOBJ()->Context()->PSSetShaderResources(_Reg, 1, &NULLPTR);
		break;
	case SHADERTYPE::SHADER_END:
		break;
	default:
...
void HTEXTURE::Create(HVECTOR _Size, DXGI_FORMAT _Fmt, UINT _BindFlag, D3D11_USAGE _eUsage) 
{
...
void HTEXTURE::Create(D3D11_TEXTURE2D_DESC _Desc) 
{
	...
	// 텍스처를 만들어주는 것.
	// m_pTex을 얻어옵니다.
	if (S_OK != HGAMEDEVICE::MAINOBJ()->Device()->CreateTexture2D(&Desc, nullptr, (ID3D11Texture2D**)&m_pTex))
	{
		assert(false);
	}
...
void HTEXTURE::SetPixel(void* _PixelData, int Size)
{
	...
	// 그 위험을 피하기 위해서 멈춰.
	// => 느려진다.
	HGAMEDEVICE::MAINOBJ()->Context()->Map(m_pTex, 0, D3D11_MAP_WRITE_DISCARD, 0, &MSUB);
	...
	HGAMEDEVICE::MAINOBJ()->Context()->Unmap(m_pTex, 0);
}
...
// 쉐이더 리소스가 불가능
	...
	{
		if (S_OK != HGAMEDEVICE::MAINOBJ()->Device()->CreateDepthStencilView(m_pTex, nullptr, &m_DSV))
		{
	...
	else 
	{
		...
		if (0 != (D3D11_BIND_SHADER_RESOURCE & Desc.BindFlags))
		{
			if (S_OK != HGAMEDEVICE::MAINOBJ()->Device()->CreateShaderResourceView(m_pTex, nullptr, &m_SRV))
			{
				AMSG(L"쉐이더 리소스 생성에 실패했습니다.");
			}
		}
		...
		{
			if (S_OK != HGAMEDEVICE::MAINOBJ()->Device()->CreateRenderTargetView(m_pTex, nullptr, &m_RTV))
			{
