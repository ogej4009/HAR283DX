...
void HVTXSHADER::Load(const HGAMESTRING& _FuncName, unsigned int _VH, unsigned int _VL)
{
	m_VH = _VH;
	m_VL = _VL;
	...
	{
		HGAMESTRING ErrorText = (char*)m_pErrBlob->GetBufferPointer();

		AMSG(ErrorText + L"쉐이더 컴파일 에러");
		return;
	}

	if (S_OK != HGAMEDEVICE::MAINOBJ()->Device()->CreateVertexShader(
...
{
	// m_ShaderPtr
	HGAMEDEVICE::MAINOBJ()->Context()->IASetInputLayout(m_pLayOut);
	HGAMEDEVICE::MAINOBJ()->Context()->VSSetShader(m_ShaderPtr, 0, 0);
}
...
{
	if (S_OK != HGAMEDEVICE::MAINOBJ()->Device()->CreateInputLayout(
		&m_ArrInputList[0],
