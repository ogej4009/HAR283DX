...
void HPIXSHADER::Load(const HGAMESTRING& _FuncName, unsigned int _VH, unsigned int _VL)
{
	m_VH = _VH;
	m_VL = _VL;
	...
	if (S_OK != HGAMEDEVICE::MAINOBJ()->Device()->CreatePixelShader(
...
{
	// m_ShaderPtr
	HGAMEDEVICE::MAINOBJ()->Context()->PSSetShader(m_ShaderPtr, 0, 0);
}