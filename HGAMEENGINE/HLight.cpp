...
void HLight::InitSetting(int _Order)
{
	...
	ShadowDepthTarget = new HRenderTarget();
	ShadowDepthTarget->CreateTarget({4096, 4096}, HVECTOR::NONE, true, DXGI_FORMAT::DXGI_FORMAT_R32_FLOAT);
...
void HLight::LightUpdate()
{
	...
	for (LightData* Data : DataList)
	{
		...
		// Data->AmbColor = HVECTOR::ONE * 0.2f;
