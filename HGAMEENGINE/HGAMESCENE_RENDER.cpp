...
void HGAMESCENE::Render()
{
	...
	// 백버퍼가 먼저 세팅 된다.
	HGAMEDEVICE::MAINOBJ()->RenderStart();
	...
	for (auto& _Cam : m_AllCam)
	{
		...
		for (int RenderOrder : _Cam.second->m_VIEWRENDERERS)
		{
			...
			std::map<int, std::list<HPTR<HRENDERER>>>::iterator RenderGroup = m_AllRender.find(RenderOrder);
	...
	HGAMEDEVICE::MAINOBJ()->BackBufferRenderTarget()->Clear(true);
	// HGAMEDEVICE::MAINOBJ()->BackBufferRenderTarget()->Setting();
	...
	for (auto& _Cam : m_AllCam)
	{
		...
		HGAMEDEVICE::MAINOBJ()->BackBufferRenderTarget()->Merge(_Cam.second->CamTarget());
		// HGAMEDEVICE::MAINOBJ()->BackBufferRenderTarget() 여기에다가 랜더링 하면 된다.
	}

	{
		H3DDEBUG::DebugRender();
	}

	HGAMEDEVICE::MAINOBJ()->DataRenderTarget()->Clear(false);

	// 포스트 프로세싱 구조.

	HGAMEDEVICE::MAINOBJ()->RenderEnd();
