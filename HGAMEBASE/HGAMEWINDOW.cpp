	...
	case EC_USERABORT:
		{
			...
			HPTR<HGAMEWINDOW> PTR = HGAMEWINDOW::Find(ArrName);
...
void HGAMEWINDOW::Create(const HGAMESTRING& _WindowName)
{
	HPTR<HGAMEWINDOW> NewWindow = CreateToInsert(_WindowName, _WindowName);
}

void HGAMEWINDOW::Create(const HGAMESTRING& _WindowName, HWND _HWnd)
{
	HPTR<HGAMEWINDOW> NewWindow = CreateToInsert(_WindowName, _HWnd);
}

HGAMEWINDOW::HGAMEWINDOW(const HGAMESTRING& _Name) : MoviePlayer(nullptr)
{
...
void HGAMEWINDOW::Size(const HVECTOR& _Size)
{
...
HVECTOR HGAMEWINDOW::MousePosTo3D()
{
	HVECTOR Pos = MousePos();
...
HVECTOR HGAMEWINDOW::WindowPosTo3DPos(HVECTOR _WindowPos)
{
	return HVECTOR::WindowPosTo3DPos(_WindowPos, m_Size);
}